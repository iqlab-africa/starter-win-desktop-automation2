...
def download_orders_files():
    try:
        ...
        csv = do_download(blob_service_client=blob_service_client, filename=orders_csv_file)
        excel = do_download(blob_service_client=blob_service_client, filename=orders_excel_file)
...
def do_download(blob_service_client, filename):
        try :
             container_client = blob_service_client.get_container_client(orders_container_name)
             ...
             print(f"{tag} do_download: local_path: {local_path} fileName: {filename} size: {file_size} bytes")
             print(f"\n")
             return local_path;
...
def load_files_to_json(file_type: str):
    print(f'\n\n{tag} loading file contents and creating json list; fileType: {file_type}\n')
    json_list = []
        ...
        for file in files:
            ...
            if file.endswith('.csv'):
                if file_type == CSV_FILE_TYPE:
                    print(f'{tag} reading {file_path} into pandas DataFrame ...')
            ...
            print(f"\n{tag} Loaded {file_path} with {len(json_data)} records")

    print(f'\n{tag} returning {len(json_list)} json records from file type: {file_type}\n\n')
    return json_list